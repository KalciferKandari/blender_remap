...
# Inputs:
# ==========
# - <string> project_path: Absolute project directory path to be recursively searched for references to the old library, and have them changed to the current '.blend' file.
...
def make_dupli_group_source(project_path, scripts_path):
    # TODO Add warning that the entire project should be backed-up before running the script.
    ...
    if (
        ...
        print('Exiting script: The active object is not an externally linked DupliGroup.')
    ...
    current_file_rel_to_proj_path = abs_into_rel_to_proj(project_path, data.filepath)
    script_path = scripts_path + "\\" + "reference_remap_by_relink.py"
    old_library_rel_to_proj_path = proj_path_utilities.rel_into_rel_to_proj(current_file_rel_to_proj_path, obj_library_path)

    # Make sure the path is inside the project directory.
    if not old_library_rel_to_proj_path:
        print("Exiting script: Can't have a path that is outside the project directory.")
        return
    ...
    for group_iter in data.groups:
        if group_iter.name == obj_dupligroup_name:
            print('Exiting script: Group "%s" already exists.' % obj_dupligroup_name)
            # Stop the script by returning.
            return

    # Go through the individual objects that are part of the DupliGroup and check where their library is, if it is the old library, abort.
    # We do this because otherwise when changing internally linked DupliGroups in the old library file to external, a circular reference would be created, so the objects that have the circular references would not turn up in the DupliGroups. This is the same even when linking normally using the GUI. This is never a desirable outcome, so we stop the script here.
    circulars = []
    for obj_iter in obj.dupli_group.objects:
        if (obj_iter.library.filepath == obj_library_path) and (not obj_iter.is_library_indirect):
            circulars.append("- '%s', of type '%s' in file '%s'." % (obj_iter.name, obj_iter.type, obj_iter.library.filepath))
    if circulars:
        print(
            "Exiting script: The following objects in this DupliGroup are local to the old library '.blend' file, which is at: " + obj_library_path + ". All the objects in the group in that file must be externally linked, otherwise circular references would be formed if there are internally linked DupliGroups in the old library, which would not appear correctly."
        )
    ...
    for obj_iter in group_data.objects:
        ...
        duplicate_linked_obj.data = linked_object.object.data.copy()  # FIXME Is this necessary?
    ...
    # Find the local group. We need to do this because the group is not returned by the operator function that creates it.
    print("----------")
    ...
    # Saving is necessary so the linking done in the other '.blend' files actually works.
    bpy.ops.wm.save_mainfile()

    # ----------
    # Search for links to original.
    # ----------
    # Could have used blendfile.py, but instead chose to loop through Blender files using an external Python script, because blendfile.py didn't have any documentation or comments.
    ...
    blend_iterate(
        ...
        project_path + '\\' + old_library_rel_to_proj_path,  # Argument to pass on.
