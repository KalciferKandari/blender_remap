...
# - <string> project_path: Absolute project directory path to be recursively searched for references to the library, and have their name changed.
...
def remap_name(new_name, data_type, project_path, scripts_path):
    ...
    err_name_exists = 'Exiting script: \'%s\' name "%s" already exists in this file.' % (data_type, new_name)
    ...
    if data_type == "GROUP":
        ...
        if (obj.dupli_group is None) or (obj.dupli_type != "GROUP"):
            print("Exiting script: In order to rename a group, a DupliGroup must be created to select it.")
        ...
        if obj.dupli_group is None:
            print("Exiting script: To rename a group, first create a local DupliGroup from it, and select that. The DupliGroup can be deleted after the script has been run.")
            # Stop the script by returning.
            return
        ...
        # Renames the group and all the internally linked DupliGroups. No need to iterate through all the DupliGroups directly.
        # DELETED Loop not necessary?
        # for group_iter in data.groups:
        #     if group_iter.name == obj.dupli_group.name:
        #         group_iter.name = new_name
        #         break
    ...
    # ----------
        ...
        for obj_iter in data.objects:  # TODO What if there is a fake mesh?
            if (obj_iter.data is not None) and (obj_iter.type == "MESH") and (obj_iter.data.name == new_name):
                print(err_name_exists)
                # Stop the script by returning.
                return
        ...
        # Renames the object data and all the internally linked objects. No need to iterate through all the objects directly.
    ...
    # ----------
        ...
        if obj.type != "MESH":
            print("Exiting script: Selected object must be of type 'MESH' to change the name of a material.")
            # Stop the script by returning.
            return
        ...
        # Renames the material data and all the internally linked objects. No need to iterate through all the objects directly.
        obj.active_material.name = new_name
    # ----------
    # If the data type given is not valid.
    # ----------
    else:
        print("Exiting script: '%s' is not a valid data type for this function, which are 'MESH', 'GROUP', or 'MATERIAL'." % data_type)
        # Stop the script by returning.
        return
    ...
    # Rename external references by iterating through the '.blend' files in the 'project_path'.
    blend_iterate(
